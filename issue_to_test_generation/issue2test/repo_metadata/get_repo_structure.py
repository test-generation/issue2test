...
def get_project_structure_from_scratch(
    repo_name, commit_id, instance_id, repo_workspace
):

    # Generate a temperary folder and add uuid to avoid collision
    repo_workspace = os.path.join(repo_workspace, str(uuid.uuid4()))

    # assert playground doesn't exist
    assert not os.path.exists(repo_workspace), f"{repo_workspace} already exists"

    # create playground
    os.makedirs(repo_workspace)

    clone_repo(repo_name, repo_workspace)
    checkout_commit(f"{repo_workspace}/{repo_to_top_folder[repo_name]}", commit_id)
    structure = create_structure(f"{repo_workspace}/{repo_to_top_folder[repo_name]}")

    # clean up
    #subprocess.run(
    #    ["rm", "-rf", f"{repo_workspace}/{repo_to_top_folder[repo_name]}"], check=True
    #)
    d = {
        "repo": repo_name,

...
from issue2test.locate.locate_source_files_with_llm import LLMLocateSourceFiles
...
def locate_source_files(args):
    ...
    for github_issue in swe_bench_dataset:
        ...
        logging.info(f"================ github issue {instance_id} ================")
        logging.info(f"repo: {project_data['repo']}")
        logging.info(f"================ related source code files for {instance_id} ================")
        ...
        if args.file_level:
            fl = LLMLocateSourceFiles(
                project_data["instance_id"],
                structure,
                problem_statement,
            )
            found_files, additional_artifact_loc_file, file_trajectory = fl.localize(
                mock=args.mock
            )
        else:
...
def main():
    ...
    parser.add_argument("--output_file",
                        type=str,
                        default="repo_loc_outputs.jsonl")

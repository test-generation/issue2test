...
def get_direct_and_aliased_imports(file_path: str, symbol_name: str):
    """
    Searches for direct and aliased imports of the specified symbol in a file.
    Args:
        file_path (str): Path to the Python file to check.
        symbol_name (str): Name of the symbol to search for in imports.
    Returns:
        list: Tuples containing module, alias, line range, and line content.
    """
    results = []
    with open(file_path, 'r') as file:
        file_content = file.readlines()  # Read file lines for easy access to specific lines
        tree = ast.parse("".join(file_content), filename=file_path)

    # Traverse AST for import statements
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            for alias in node.names:
                if alias.name == symbol_name or alias.asname == symbol_name:
                    ...
                    line_text = file_content[node.lineno - 1].strip()  # Capture the exact line content
                    results.append((full_module, alias_name, line_range, line_text))
        elif isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name == symbol_name or alias.asname == symbol_name:
                    line_range = (node.lineno, node.end_lineno)
                    line_text = file_content[node.lineno - 1].strip()
                    results.append((alias.name, alias.asname, line_range, line_text))

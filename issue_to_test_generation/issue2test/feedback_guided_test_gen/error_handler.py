...
from feedback_guided_test_gen.docker_test_runner import DockerTestRunner, naive_contains_error
from feedback_guided_test_gen.eval_helper import parse_log_sympy
from feedback_guided_test_gen.feedback_loop import remove_tests_prefix
from feedback_guided_test_gen.tools.test_output_analyzer import parse_pytest_summary
from feedback_guided_test_gen.utils import save_iteration_data, parse_json_with_markers, \
...
class ErrorHandler:
    def __init__(self, model,
        ...
        self.SWE_BENCH_DOCKER_DIR = configuration["SWE_BENCH_DOCKER_DIR"]
        self.WORKSPACE_DIR = configuration["WORKSPACE_DIR"]
    ...
    def handle_compilation_and_runtime_error(self,
                                             test_execution_logs,
                                             source_code,
                                             test_code,
                                             iteration,
        ...
        while retries < self.retry_limit:
            ...
            logging.info(
                f"Handling compilation error attempt {retries + 1} for step {runtime_state_tracking.step_count}...")

            # Create a workspace for this refinement attempt
            step = f"step_{runtime_state_tracking.step_count}-error_fix"
            ...
            # SWE_BENCH_DOCKER_PATH = config.SWE_BENCH_DOCKER_DIR
            SWE_BENCH_DOCKER_PATH = self.SWE_BENCH_DOCKER_DIR
            ...
            if test_failures:
                ...
                error_category, reason, root_cause, repair_steps, tdd_related = self.categorize_error_tdd(
                    result_content, iteration, github_issue, test_code)
                ...
                if error_category == 'assertion':
                    ...
                    logging.info("Check if it is an assertion error and related to the issue...")
                    is_related_to_issue = self.is_assertion_error(result_content,
                                                                  test_case_generator.github_issue_id,
                                                                  test_case_generator.github_issue,
                                                                  runtime_state_tracking.step_count)

                    is_direct_match = is_related_to_issue.get("is_direct_match", False).lower() == "yes"
                    if is_direct_match:
                        logging.info("Assertion failure is not directly related to the GitHub issue. "
                                     "Refining test case...")
                        return True, True, generated_fixed_code
    ...
    def is_assertion_error(self,
        ...
        rendered_user_prompt = user_template.render(
            ...
            synthesized_test_case=synthesized_test_case,
            github_issue=github_issue_description
        )
        ...
        # Save input/output for this iteration
        step = "step_{}_check_assertion".format(runtime_state_tracking.step_count)
        save_iteration_data_for_step(self.trajectory_folder, step, rendered_user_prompt, response)
    ...
    def categorize_error(self, result_content, test_file):
        """
        ...
        """
        self.logger.info(f"Categorizing error for iteration {runtime_state_tracking.step_count}...")
        runtime_state_tracking.step_count += 1
        ...
        step = "step-{}-error_categorization".format(runtime_state_tracking.step_count)
        # Save input/output for this iteration
        save_iteration_data_for_step(self.trajectory_folder, step, rendered_user_prompt, response)
    ...
    def categorize_error_tdd(self, result_content, iteration, github_issue, test_file):
        ...
        step = "step-{}-error_categorization_tdd".format(runtime_state_tracking.step_count)
        # Save input/output for this iteration
        save_iteration_data_for_step(self.trajectory_folder, step, rendered_user_prompt, response)
    ...
    def refine_test_case_until_related_assertion_generated(self, test_case_generator,
        ...
        """
        logging.info("Refining test case until related assertion is generated...")
        project_name = test_case_generator.project_name
        generated_tests_folder = utils.get_generated_tests_folder(github_issue['instance_id'])
        ...
        while refinement_attempts < max_refinements:
            logging.info(
                f"Attempt to generate an assertion that fails: {refinement_attempts + 1}, step {runtime_state_tracking.step_count}...")

            # Create a workspace for this refinement attempt
            refinement_workspace = os.path.join(trajectory_folder,
                                                f"step_{runtime_state_tracking.step_count}")
            ...
            # Check the test results
            log_detected_failures, results_summary, test_log_output = docker_runner.check_test_results(refinement_workspace)

            if log_detected_failures:
                logging.info("Refined test failed. Categorizing the error...")

                # Categorize the error (it could be compilation or assertion)
                error_category = self.categorize_error(results_summary, test_code)
                ...
                if error_category == 'compilation':
                    ...
                    is_compilation_fixed, fix = self.handle_compilation_and_runtime_error(
                        test_execution_logs=results_summary,
                    ...
                    is_related_to_issue = self.is_assertion_error(results_summary,
                                                                  test_case_generator.github_issue_id,
                                                                  test_case_generator.github_issue, iteration)
                    is_related = is_related_to_issue.get("is_direct_match", False).lower() == "yes"
    ...
    def refine_test_until_failure(self,
        ...
        while refinement_attempts < max_refinements:
            ...
            logging.info(
                f"Refinement attempt {refinement_attempts + 1} for step {runtime_state_tracking.step_count}...")

            # Create a workspace for this refinement attempt
            step = f"step_{runtime_state_tracking.step_count}-test_refinement"
            refinement_workspace = os.path.join(trajectory_folder, f"step_{runtime_state_tracking.step_count}")
            if not os.path.exists(refinement_workspace):
                os.makedirs(refinement_workspace)
            ...
            # Run the refined test in Docker
            issue_id = test_case_generator.github_issue_id
            docker_runner = DockerTestRunner(self.SWE_BENCH_DOCKER_DIR,
                                             new_test_file_path,
                                             issue_id,
                                             project_name=test_case_generator.project_name)
            docker_runner.run_tests_in_docker(generated_tests_folder, relative_path_to_docker)
            ...
            log_detected_failures, results_summary, test_log_output = docker_runner.check_test_results(
                generated_tests_folder)
            log_detected_failures = log_detected_failures or naive_contains_error(test_log_output)
            ...
            if is_test_failure:
                ...
                logging.info("Test now fails as expected (TDD).")
                status_data["steps"][-1]["test_failures"] = True

                error_category, reason, root_cause, repair_steps = self.categorize_error(test_log_output,
                                                                                         test_code)
                if error_category == 'compilation' or error_category == 'runtime':
                    ...
                    is_related_to_issue = self.is_assertion_error(test_log_output,
                                                                  test_case_generator.github_issue_id,
                                                                  test_case_generator.github_issue,
                                                                  iteration)
                    is_direct_match = is_related_to_issue.get("is_direct_match", False).lower() == "yes"
